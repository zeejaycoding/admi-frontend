...
const RegionWelcomeSection = ({ config }) => {
  const { t } = useTranslation('ui');
  const { welcome, regionParam } = config;
  const images = [
    { src: homeAboutUs, alt: 'Powerful Teaching', className: 'rounded-tl-lg' },
    { src: homeOurLeaders, alt: 'Our Community', className: 'rounded-tr-lg' },
    { src: homeCommunity, alt: 'Fellowship', className: 'rounded-bl-lg' },
    { src: homeCourses, alt: 'Bible Study', className: 'rounded-br-lg' },
  ];
      ...
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="space-y-8">
            <div className="space-y-4">
              <div className="inline-flex items-center gap-2 text-primary-600 font-semibold">
                ...
                <span>{t(welcome.badgeKey)}</span>
              </div>
              <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-gray-900 leading-tight">
                {t(welcome.headingKey)}
              </h2>
              <p className="text-xl text-gray-600 leading-relaxed">
                {t(welcome.subtitleKey)}
              </p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Link to={`/campuses?region=${regionParam}`} className="btn-primary text-center">
                {t('welcome.findCampus')}
              </Link>
              <Link to={`/about?region=${regionParam}`} className="btn-secondary text-center">
                {t('welcome.learnMore')}
              </Link>
            </div>
          </div>
          ...
          <div className="relative">
            ...
            {/* Decorative Elements */}
            <div className={`absolute -top-4 -right-4 w-8 h-8 ${welcome.decorative[0]} rounded-full opacity-80 animate-pulse`}></div>
            <div className={`absolute -bottom-4 -left-4 w-6 h-6 ${welcome.decorative[1]} rounded-full opacity-60 animate-bounce`}></div>
          </div>
        </div>
      </div>
    </section>
...
export default RegionWelcomeSection;
