...
const PbsHonorOfferingPage = () => {
  ...
  const handlePayPalReturn = useCallback(async (orderId) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setSuccess(t('pbs.successPaypalOffering', { currency, amount: captured }));
        setPaypalAmount('50');
        setPaypalEmail('');
        window.history.replaceState({}, document.title, '/power-bible-school/honor-offering');
  ...
  useEffect(() => {
    ...
    if (payment === 'success' && sessionId) {
      setSuccess(t('pbs.successStripeOffering'));
      setStripeAmount('50');
      setStripeEmail('');
      window.history.replaceState({}, document.title, '/power-bible-school/honor-offering');
      window.scrollTo({ top: 0, behavior: 'smooth' });
  ...
  const handleStripeCheckout = async () => {
    ...
    try {
      ...
      const res  = await axios.post(`${API_BASE_URL}/donations/create-stripe-session`, {
        ...
        currency:     'USD',
        offeringType: 'PBS_HONOUR_OFFERING',
        donorEmail:   stripeEmail,
        successUrl:   `${base}/power-bible-school/honor-offering?payment=success&session_id={CHECKOUT_SESSION_ID}`,
        cancelUrl:    `${base}/power-bible-school/honor-offering?payment=cancelled`,
      });
      ...
      <div className="container mx-auto px-4 max-w-4xl">
        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 bg-primary-50 border border-primary-200 text-primary-700 text-xs font-semibold tracking-widest uppercase px-4 py-1.5 rounded-full mb-5">
            {t('pbs.honorOfferingBadge')}
          </div>
          <h1 className="text-2xl sm:text-4xl font-bold text-primary-600 mb-4">{t('pbs.honorOfferingTitle')}</h1>
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            {t('pbs.honorOfferingDesc')}
          </p>
        </div>
        ...
        {success && (
          <div className="mb-8 p-5 bg-green-50 border border-green-400 rounded-xl text-center">
            ...
            <button
              onClick={() => navigate('/power-bible-school')}
...
export default PbsHonorOfferingPage;
